...
def train(args, train_dataset, model, discriminators, tokenizer):
    ...
    scheduler_D = get_linear_schedule_with_warmup(optimizer_D, num_warmup_steps=args.warmup_steps, num_training_steps=t_total)
    if args.fp16:
        try:
            from apex import amp
        except ImportError:
            raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use fp16 training.")
        [model, discriminators], [optimizer, optimizer_D] = amp.initialize([model, discriminators] [optimizer, optimizer_D], opt_level=args.fp16_opt_level)
...
def update_D(args, model, outputs, optimizer, scheduler, step):
    ...
    if (step + 1) % args.gradient_accumulation_steps == 0:
        ...
        if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
            # Log metrics
            tb_writer.add_scalar('lr_D', scheduler.get_lr()[0], global_step)
            tb_writer.add_scalar('loss_D', (tr_loss - logging_loss)/args.logging_steps, global_step)
            logging_loss = tr_loss
